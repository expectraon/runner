...
//		GUILayout.Space(6);
	...
	private void applyScaleLifetime()
	{
		foreach(GameObject go in Selection.gameObjects)
		{
			...
			{
                var psMain = ps.main;
                psMain.simulationSpeed = (100.0f / LTScalingValue);
			}
		}
	}
	...
	private void CopyModules(ParticleSystem source, ParticleSystem dest)
	{
		...
		if(b_modules[0])
		{
            ...
            var destEmission = dest.emission;
            var sourceEmission = source.emission;

            destEmission.rateOverTime = sourceEmission.rateOverTime;
	...
	private void ScaleParticleValues(ParticleSystem ps, GameObject parent)
	{
        //Particle System
        var psMain = ps.main;
        psMain.startSizeMultiplier *= ScalingValue;
        psMain.gravityModifierMultiplier *= ScalingValue;
		if(psMain.startSpeedMultiplier > 0.01f) psMain.startSpeedMultiplier *= ScalingValue;
